...
This is a Discord bot designed to facilitate interactive quizzes within a Discord server.
The bot allows users to start quizzes, answer questions, and view results in real-time.
It supports multiple quizzes, each defined by a JSON file, and provides a user-friendly
interface for managing and participating in quizzes.
...
- **`!start_quiz <quiz_name>`**: Starts a quiz with the specified name.
  To allow users to select multiple answers, use **`!start_quiz <quiz_name> True`**.
- **`!next_question`**: Moves to the next question. Only the quiz starter can use this command.
...
2. **Answer Questions**:
   The bot will post questions with interactive buttons for each option.
   Users can click on the buttons to vote.
...
This project uses [pre-commit](https://pre-commit.com/) to enforce quiz data quality.

After cloning, install the hooks with:
...
The hooks verify that all quiz answer options are at most 80 characters and that
code blocks (triple backticks) only appear inside questions, not in answer options.

## Contributing

Contributions are welcome! Please fork the repository and submit a pull request with
your changes, or support the project on Buy Me a Coffee.
